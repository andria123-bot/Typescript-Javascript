...
class Assault extends WeaponClass {
  constructor(
    name: string,  
    damage: number,
    ammo: number,
    ...
    super(name, damage, ammo, maxAmmo)
  }
}

class Support extends WeaponClass {
  constructor(
    name: string,  
    damage: number,
    ammo: number,
...
class Sniper extends WeaponClass {
  constructor(
    name: string,
    damage: number,
    ammo: number,
...
const weapons = {
  Assault: {
    [1]: {name: "M4A1", damage: 30, ammo: 30, maxAmmo: 30},
    [2]: {name: "AK-47", damage: 35, ammo: 30, maxAmmo: 30},
    [3]: {name: "FAMAS", damage: 28, ammo: 25, maxAmmo: 25},
  },

  Support: {
    [1]: {name: "M249", damage: 25, ammo: 100, maxAmmo: 100},
    [2]: {name: "RPD", damage: 27, ammo: 75, maxAmmo: 75},
    [3]: {name: "PKM", damage: 30, ammo: 100, maxAmmo: 100},
  },

  Sniper: {
    [1]: {name: "AWP", damage: 90, ammo: 10, maxAmmo: 10},
    [2]: {name: "Dragunov", damage: 80, ammo: 20, maxAmmo: 20},
    [3]: {name: "The Fix", damage: 65, ammo: 15, maxAmmo: 15},
  },
}

let isSelecting = true
let selectedClass: string | null = null
let selectedClassWeapons
let selectedWeapon: WeaponClass | null = null


while (isSelecting){
  const classChoice = window.prompt("Enter Your Class (Assault, Support, Sniper): ")
  const normalizedChoice = classChoice ? classChoice.charAt(0).toUpperCase() + classChoice.slice(1).toLowerCase() : null

  if (normalizedChoice && ["Assault", "Support", "Sniper"].indexOf(normalizedChoice) !== -1) {
    selectedClass = normalizedChoice;

    const weaponList = weapons[selectedClass as keyof typeof weapons];
    const weaponNames = Object.keys(weaponList).map(key => weaponList[key as any].name).join(", ");
    selectedClassWeapons = weaponNames;

    const gunChoice = window.prompt(`Please Choice Weapon From ${normalizedChoice} Class! ${selectedClassWeapons}`)
    
    let chosenWeapon = null;
    if (gunChoice) {
      for (const key in weaponList) {
        const weapon = weaponList[key as any];
