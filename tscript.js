...
var WeaponClass = /** @class */ (function () {
    function WeaponClass(name, damage, ammo, maxAmmo) {
        this.name = name;
        this.damage = damage;
        this.ammo = ammo;
...
var Assault = /** @class */ (function (_super) {
    __extends(Assault, _super);
    function Assault(name, damage, ammo, maxAmmo) {
        return _super.call(this, name, damage, ammo, maxAmmo) || this;
    }
    return Assault;
}(WeaponClass));
var Support = /** @class */ (function (_super) {
    __extends(Support, _super);
    function Support(name, damage, ammo, maxAmmo) {
        return _super.call(this, name, damage, ammo, maxAmmo) || this;
    }
    return Support;
}(WeaponClass));
var Sniper = /** @class */ (function (_super) {
    ...
    function Sniper(name, damage, ammo, maxAmmo) {
        return _super.call(this, name, damage, ammo, maxAmmo) || this;
    }
    return Sniper;
}(WeaponClass));
var weapons = {
    Assault: (_a = {},
        _a[1] = { name: "M4A1", damage: 30, ammo: 30, maxAmmo: 30 },
        _a[2] = { name: "AK-47", damage: 35, ammo: 30, maxAmmo: 30 },
        _a[3] = { name: "FAMAS", damage: 28, ammo: 25, maxAmmo: 25 },
        _a),
    Support: (_b = {},
        _b[1] = { name: "M249", damage: 25, ammo: 100, maxAmmo: 100 },
        _b[2] = { name: "RPD", damage: 27, ammo: 75, maxAmmo: 75 },
        _b[3] = { name: "PKM", damage: 30, ammo: 100, maxAmmo: 100 },
        _b),
    Sniper: (_c = {},
        _c[1] = { name: "AWP", damage: 90, ammo: 10, maxAmmo: 10 },
        _c[2] = { name: "Dragunov", damage: 80, ammo: 20, maxAmmo: 20 },
        _c[3] = { name: "The Fix", damage: 65, ammo: 15, maxAmmo: 15 },
        _c),
};
var isSelecting = true;
var selectedClass = null;
var selectedClassWeapons;
var selectedWeapon = null;
var _loop_1 = function () {
    var classChoice = window.prompt("Enter Your Class (Assault, Support, Sniper): ");
    var normalizedChoice = classChoice ? classChoice.charAt(0).toUpperCase() + classChoice.slice(1).toLowerCase() : null;
    if (normalizedChoice && ["Assault", "Support", "Sniper"].indexOf(normalizedChoice) !== -1) {
        selectedClass = normalizedChoice;
        var weaponList_1 = weapons[selectedClass];
        var weaponNames = Object.keys(weaponList_1).map(function (key) { return weaponList_1[key].name; }).join(", ");
        selectedClassWeapons = weaponNames;
        var gunChoice = window.prompt("Please Choice Weapon From ".concat(normalizedChoice, " Class! ").concat(selectedClassWeapons));
        var chosenWeapon = null;
        if (gunChoice) {
            for (var key in weaponList_1) {
                var weapon = weaponList_1[key];
                if (weapon.name.toLowerCase() === gunChoice.toLowerCase()) {
                    chosenWeapon = weapon;
                    break;
                }
            }
        ...
        if (chosenWeapon) {
            switch (selectedClass) {
                case "Assault":
                    selectedWeapon = new Assault(chosenWeapon.name, chosenWeapon.damage, chosenWeapon.ammo, chosenWeapon.maxAmmo);
                    break;
                case "Support":
                    selectedWeapon = new Support(chosenWeapon.name, chosenWeapon.damage, chosenWeapon.ammo, chosenWeapon.maxAmmo);
                    break;
                case "Sniper":
                    selectedWeapon = new Sniper(chosenWeapon.name, chosenWeapon.damage, chosenWeapon.ammo, chosenWeapon.maxAmmo);
                    break;
            }
            ...
            console.log("Invalid Weapon Choice, Please Try Again!");
        }
    }
    else {
        console.log("Invalid Class Choice, Please Try Again!");
    }
};
while (isSelecting) {
    _loop_1();
